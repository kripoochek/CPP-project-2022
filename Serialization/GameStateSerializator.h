...
struct GameStateSerializator {
    static void deserializeMap(std::shared_ptr<b2World> world, serialized::Map &serializedMap, std::shared_ptr<Map> gameMap, sf::Texture& verticalTexture, sf::Texture& horizontalTexture);
    ...
    static serialized::Map serializeMap(std::shared_ptr<RoundState> state);
    private:
