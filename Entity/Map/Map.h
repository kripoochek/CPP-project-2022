...
struct Map {
    ...
    Map(std::shared_ptr<b2World> world, int rows, int columns, sf::Vector2f coordinates, sf::Texture& texture_, sf::Texture& verticalTexture, sf::Texture& horizontalTexture);
    int rows = randNum(5, 6); // 5 6
    int columns = randNum(8, 10); //8 10
