...
Map::Map(int rows, int columns, sf::Vector2f coordinates, sf::Texture& texture_, sf::Texture& verticalTexture, sf::Texture& horizontalTexture) : rows(rows), columns(columns) {
    for (int i = 0; i < rows; i++) {
        map.push_back(std::vector<std::shared_ptr<Box>>{
                std::make_shared<Box>(coordinates, texture_)});
        coordinates = sf::Vector2f(coordinates.x, coordinates.y + 100);
